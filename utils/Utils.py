...
# def time_positional_encoding(time_tensor, d_model):
#     pe = torch.zeros_like(time_tensor, dtype=torch.float32, device=time_tensor.device).unsqueeze(-1).repeat(1, 1, d_model)
#     position = time_tensor.unsqueeze(-1).float()
#     div_term = torch.exp(torch.arange(0, d_model, 2, device=time_tensor.device).float() * -(math.log(10000.0) / d_model))
